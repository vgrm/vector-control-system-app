...
const ArcHandleList = ({ classes, ...props }) => {

    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(5);

    const emptyRows = rowsPerPage - Math.min(rowsPerPage, props.arcsHandleList.length - page * rowsPerPage);
        ...
        <Container>
            ...
            <TableContainer component={Paper}>
                <Table size="small" aria-label="a dense table">
                    ...
                    <TableBody>
                        {
                            (rowsPerPage > 0 ? props.arcsHandleList.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : props.arcsHandleList)
                                .map((arc, index) => {
                                    return (<TableRow key={index} hover>
                                        <TableCell>{arc.handle}</TableCell>
                                        <TableCell>{arc.projectId}</TableCell>
                                        <TableCell>{arc.project.name}</TableCell>
                                        <TableCell>{arc.project.name}</TableCell>
                    ...
                    <TableFooter>
                        <TableRow>
                            <TablePagination
                                ...
                                colSpan={10}
                                count={props.arcsHandleList.length}
...
const mapStateToProps = state => ({
    arcsHandleList: state.arc.listHandle
})

const mapActionToProps = {
    fetchArcsHandle: actions.fetchArcsHandle
}

export default connect(mapStateToProps, mapActionToProps)(withStyles(styles)(ArcHandleList));